(function(){let e=document.createElement(`link`).relList;if(e&&e.supports&&e.supports(`modulepreload`))return;for(let e of document.querySelectorAll(`link[rel="modulepreload"]`))n(e);new MutationObserver(e=>{for(let t of e)if(t.type===`childList`)for(let e of t.addedNodes)e.tagName===`LINK`&&e.rel===`modulepreload`&&n(e)}).observe(document,{childList:!0,subtree:!0});function t(e){let t={};return e.integrity&&(t.integrity=e.integrity),e.referrerPolicy&&(t.referrerPolicy=e.referrerPolicy),e.crossOrigin===`use-credentials`?t.credentials=`include`:e.crossOrigin===`anonymous`?t.credentials=`omit`:t.credentials=`same-origin`,t}function n(e){if(e.ep)return;e.ep=!0;let n=t(e);fetch(e.href,n)}})(),((e,t)=>()=>(t||(e((t={exports:{}}).exports,t),e=null),t.exports))((()=>{var e=[];async function t(){try{e=await(await fetch(`/events_2025/data.json`)).json(),n(e),l()}catch(e){console.error(`Error loading data:`,e)}}function n(e){let t=document.getElementById(`wedding-container`),n=document.getElementById(`obituary-container`),r=e.filter(e=>e.type===`wedding`),i=e.filter(e=>e.type===`obituary`);a(r,t,`경사`),a(i,n,`조사`),o(e)}function r(e){if(!e)return 1e5;let t=e.trim();return t.includes(`친척`)?3e5:t.includes(`직장 동료`)||t.includes(`교회 지인`)?15e4:(t.includes(`학교선후배`)||t.includes(`동호회 지인`),1e5)}function i(e){return e.toLocaleString()}function a(e,t,n){if(!t)return;if(e.length===0){t.innerHTML=`<p style="padding: 20px; color: #8B95A1;">데이터가 없습니다.</p>`;return}let a={},o=0;e.forEach(e=>{let t=(e.type===`wedding`?e.dateTime:e.diedDate).match(/(\d{4})[-년]\s*(\d{1,2})/),n=t?`${t[1]}년 ${parseInt(t[2])}월`:`기타`;a[n]||(a[n]=[]),a[n].push(e),o+=r(e.relation)});let s=Object.keys(a).sort((e,t)=>{let n=e.match(/(\d+)년\s+(\d+)월/),r=t.match(/(\d+)년\s+(\d+)월/);return!n||!r?0:parseInt(n[1])*100+parseInt(n[2])-(parseInt(r[1])*100+parseInt(r[2]))}),c=`
        <table>
            <thead>
                <tr>
            ...
            <tbody>
    `,l=1;s.forEach(e=>{let t=e.split(` `)[1]||e;a[e].forEach(e=>{let n=e.attachment?`<span class="attachment-icon" onclick="event.stopPropagation(); showImage('${e.attachment}')" title="이미지 보기">📎</span>`:`-`,a=r(e.relation);c+=`
        ...
        </table>`,t.innerHTML=c}function o(e){let t=document.getElementById(`total-amount`);t&&(t.textContent=i(e.reduce((e,t)=>e+r(t.relation),0))+`원`)}function s(t){let n=e.find(e=>e.id===t);if(!n)return;let a=document.getElementById(`modal`),o=document.getElementById(`modal-body`),s=r(n.relation);n.type===`wedding`?o.innerHTML=`
